...
export function RichTextEditor({
  ...
  async function onPickImage(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImgErr('Falha ao enviar a imagem.');
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <div className="border border-subtle rounded-md bg-canvas overflow-hidden">
      ...
      >
        ...
        <span className="w-px h-4 bg-[var(--border-subtle)] mx-1" />
        <button type="button" className={btn(editor.isActive('link'))} onClick={openLinkDialog} title="Link"><Link2 size={15} /></button>
      ...
      {imgErr && <div className="px-3 py-1 text-[12px] text-[var(--red-400)]">{imgErr}</div>}
